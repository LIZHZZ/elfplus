...
public class ElfAdaptXORCompressor {
    private int storedLeadingZeros = Integer.MAX_VALUE;
    private int storedTrailingZeros = Integer.MAX_VALUE;
    private int storedCenterBits = Integer.MAX_VALUE;
    private boolean first = true;
    private int size;
    private long storedVal = 0;
    private final int windowSize = 16 / 2;
    ...
    private int compressValue(long value) {
        ...
        if (xor == 0) {
            ...
            if (leadingZeros == storedLeadingZeros && trailingZeros >= storedLeadingZeros) {
                // case 00
                ...
                int centerBits = 64 - storedTrailingZeros - storedTrailingZeros;

                if (centerBits > storedCenterBits - windowSize && centerBits <= storedCenterBits + windowSize) {
                    // case 10
                    out.writeInt((((0x2 << 3) | leadingRepresentation[storedLeadingZeros]) << 4) | (centerBits - storedCenterBits + windowSize & 0xf), 9);
