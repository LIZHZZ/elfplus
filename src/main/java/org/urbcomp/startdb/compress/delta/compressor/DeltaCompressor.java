package org.urbcomp.startdb.compress.delta.compressor;
...
public class DeltaCompressor {
    ...
    private double storeV;
    private int[] lastAlphaAndBetaStar;
    private boolean first = true;
    private int lastGAlpha;
    ...
    public void addValue(double v) {
        ...
        long vPrimeLong;
        double deltaV = storeV - v;
        long deltaLong = Double.doubleToRawLongBits(deltaV);
        if (first) {
            storeV = v;
            first = false;
            lastAlphaAndBetaStar = getAlphaAndBetaStar(v);
            int e = ((int) (vLong >> 52)) & 0x7ff;
            lastGAlpha = getFAlpha(lastAlphaAndBetaStar[0]) + e - 1023;
            return;
        }
...
//            size += writeBit(false);
            ...
            long e = ((vLong >> 52)) & 0x7ff;
            long eOfDelta = ((deltaLong >> 52)) & 0x7ff;


            int gAlpha = getFAlpha(alphaAndBetaStar[0]) + (int)e - 1023;

            int eraseBits;

            if (gAlpha > lastGAlpha) {
                eraseBits = 52 - gAlpha;
            }else {
                eraseBits = 52 - lastGAlpha;
            }
//
//            int eraseBits = 52 - gAlpha;
            long mask = 0xffffffffffffffffL << eraseBits;
            System.out.println(Long.toBinaryString(mask));
            long delta = (mask) & ((deltaLong& DoubleConsts.SIGNIF_BIT_MASK) >>(e-eOfDelta));
            System.out.println(Long.toBinaryString(delta));
            System.out.println("deltalong");
            System.out.println(Long.toBinaryString(deltaLong));

            int leadingZeros = Long.numberOfLeadingZeros(delta);
            int trailingZeros = Long.numberOfTrailingZeros(delta);
            int centerBits = 64 - leadingZeros - trailingZeros;
            first = false;
            lastAlphaAndBetaStar = alphaAndBetaStar;
            lastGAlpha = gAlpha;
        }
        storeV = v;
